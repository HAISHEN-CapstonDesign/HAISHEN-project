...
export default {
    created(){
        ...
        // axios    
        //     .post('http://localhost:3000/api/getpostlist', { genre: '건축' }, { headers: {'token': localStorage.getItem('access_token')}})
        //         .then(res => {
        ...
        axios    
            .post('http://localhost:3000/api/getrecruitingPost', { genre: '건축' }, { headers: {'token': localStorage.getItem('access_token')}})
                .then(res => {
    ...
    data(){
        return{
            ...
            bookList: [
                {
                    title:'아시아선수촌아파트 건축문화투어의 기억',
                    info:'아시아선수촌아파트 건축문화투어의 기억',
                    src: require('../../assets/contents_picture/build01.jpg'),
                    members: ['황자양'],
                    show: false,
                    like: 3,
                    subscribe: 2,
                },
                {
                    title:'건축=인생을 배우다.',
                    info:'건축=인생을 배우다.',
                    src: require('../../assets/contents_picture/build02.jpg'),
                    members: ['요리하고','경애'],
                    show: false,
                    like: 48,
                    subscribe: 29,
