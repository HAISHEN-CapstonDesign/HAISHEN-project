...
export default {
     created(){
        ...
        axios    
            .post('http://localhost:3000/api/getpostlist', { genre: '시사' }, { headers: {'token': localStorage.getItem('access_token')}})
                .then(res => {
        ...
        axios    
            .post('http://localhost:3000/api/getrecruitingPost', { genre: '시사' }, { headers: {'token': localStorage.getItem('access_token')}})
                .then(res => {
            ...
            bookList: [
                {
                    title:'차별금지법? 종교는 예외죠',
                    info:'차별금지법? 종교는 예외죠',
                    src: require('../../assets/contents_picture/pre01.jpg'),
                    members: ['위치캣'],
                    show: false,
                    like: 3,
                    subscribe: 2,
                ...
                {
                    title:'코로나 확진자 950명, K-방역은 결국 무너졌다.',
                    info:'코로나 확진자 950명, K-방역은 결국 무너졌다.',
                    src: require('../../assets/contents_picture/pre02.jpg'),
                    members: ['온유'],
                    show: false,
                    like: 48,
                    subscribe: 29,
