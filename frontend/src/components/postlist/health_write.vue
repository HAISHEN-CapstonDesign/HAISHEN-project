...
export default {
     created(){
        ...
        axios    
            .post('http://localhost:3000/api/getpostlist', { genre: '건강' }, { headers: {'token': localStorage.getItem('access_token')}})
                .then(res => {
        ...
        axios    
            .post('http://localhost:3000/api/getrecruitingPost', { genre: '건강' }, { headers: {'token': localStorage.getItem('access_token')}})
                .then(res => {
    ...
    data(){
        return{
            ...
            bookList: [
                {
                    title:'코로나 격리 중 정신 건강을 관리하는 팁 10가지',
                    info:'코로나 격리 중 정신 건강을 관리하는 팁 10가지',
                    src: require('../../assets/contents_picture/health_corona.jpg'),
                    members: ['김지수'],
                    show: false,
                    like: 3,
                    subscribe: 2,
                ...
                {
                    title:'눈 건강을 지키는 방법을 알아보자!',
                    info:'눈 건강을 지키는 방법을 알아보자!',
                    src: require('../../assets/contents_picture/health_eye.jpg'),
                    members: ['서울시교육청'],
                    show: false,
                    like: 48,
                    subscribe: 29,
